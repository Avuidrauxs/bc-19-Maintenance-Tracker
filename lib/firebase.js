...
var getAllCurrentRepairs = function() {
  ...
  for (staffs in data) {
    if (data[staffs].mainte.active === true && data[staffs].mainte.type === "repair") {
      ++allRequests;
    }
  }
...
var getAllStaffOffline = function() {
  ...
  var allStaff = 0;
  for (staffs in data) {
    if (data[staffs].staff.presence === true || data[staffs].staff.presence ==="true") {
      ++allStaff;
    }
  }
...
var getAllCurrentMaintenance = function() {
  ...
  for (staffs in data) {
    if (data[staffs].mainte.active === true && data[staffs].mainte.type === "maintenance") {
      ++allRequests;
    }
  }
...
var updateManRep = function (mant){
  ...
  var p = maintenanceController.child(mant.fKey +'/mainte').update(
{
  active : true
}
...
var updateManRepDone = function (mant){
  mant = new maintenance.Maintenance();
  var p = maintenanceController.child(mant.fKey).child('mainte').update(
{
  active : true
}
